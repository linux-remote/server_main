...
function initTmpFolder(cb){
  ...
  const mkSessDir = cb => exec('mkdir -m=777 ' + CONF.SESS_PATH, cb);
  const chSessMod = cb => exec('chmod +t ' + CONF.SESS_PATH, cb);

  const mkDustbinDir = cb => exec('mkdir -m=777 ' + CONF.DUSTBIN_PATH, cb);
  ...
  sas([mkdir,
    {
      chmod,
      sess: [mkSessDir, chSessMod],
      dustbin: [mkDustbinDir, chDustbinMod]
    },
    writeInitJSON
  ], cb);
}
