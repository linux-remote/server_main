
// 主要用来生成ssl自签名证书的。


var fs = require('fs');
var path = require('path');
var exec = require('child_process').exec;
var uid = require('uid-safe');
var sas = require('sas');
var sasFs = require('./sas-fs');
var $ssl = require('./ssl');

const CONF = global.CONF;
const DATA_PATH = CONF.DATA_PATH;
const pathObj = path.parse(DATA_PATH);
var initJSON = null;

function initDataDir(callback){

  const SSS_INDEX_DATA = JSON.stringify({
    "CAInit": false,
    "CN": "",
    "CADownloadedCount": 0
  });

  const INIT_JSON_DATA = JSON.stringify({
    "tmp": false,
    "sessionSecret": uid.sync(30)
  });

  const TPL_DATA = {
    [pathObj.base]: [null,
      {
        "ssl-self-signed": [null, {
          "index.json" : SSS_INDEX_DATA
        }],
        "init.json" : INIT_JSON_DATA
      }
    ]
  }

  sasFs.mkTree(pathObj.dir, TPL_DATA, callback)
}

function statDataDir(callback){
  fs.stat(DATA_PATH, function(err){
    if(err){
      if(err.code === 'ENOENT'){
        initDataDir(callback);
      }else{
        callback(err);
      }
    }else{
      callback();
    }
  })
}

function readInitJson(cb){arguments
  initJSON = require(DATA_PATH + '/init.json');
  this.sessionSecret = initJSON.sessionSecret;
  cb();
}
function initTmpFolder(cb){
  if(initJSON.tmp){
    return cb();
  }

  // const TMP_DATA = {
  //   "linux-remote": [null,
  //     {
  //       "session": [null],
  //       "trash" : [null]
  //     }
  //   ]
  // }
  //const chmod = cb => exec('chmod +t ' + CONF.TMP_PATH, cb);
  //const chSessMod = cb => exec('chmod +t ' + CONF.SESS_PATH, cb);
  //const chDustbinMod = cb => exec('chmod +t ' + CONF.DUSTBIN_PATH, cb);
  const mkdir = cb => exec('mkdir -m=1777 ' + CONF.TMP_PATH, cb);


  const mkSessDir = cb => exec('mkdir -m=1777 ' + CONF.SESS_PATH, cb);


  const mkDustbinDir = cb => exec('mkdir -m=1777 ' + CONF.DUSTBIN_PATH, cb);


  const writeInitJSON = function(cb){
    initJSON.tmp = true;
    fs.writeFile(DATA_PATH + '/init.json', JSON.stringify(initJSON), function(err){
      if(err){
        initJSON.tmp = false;
        cb(err);
      }else{
        cb();
      }
    })
  }

  sas([mkdir,
    {
      sess: mkSessDir,
      dustbin: mkDustbinDir
    },
    writeInitJSON
  ], cb);
}

//const chMainMod = cb => exec('chmod 700 ' + process.mainModule.filename, cb);

module.exports = function(callback){
  sas([
    statDataDir,
    readInitJson,
    initTmpFolder,
    // {
    //   chMainMod,
    //   initTmpFolder
    // },
    $ssl,
  ], callback);
}
